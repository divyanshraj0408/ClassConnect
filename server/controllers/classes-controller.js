...
const createClass = async (req, res, next) => {
  ...
  res.status(201).json({ createClass: createdClass });
};

const joinClass = (req, res, next) => {
  // /api/classes/:classId ==> to join a class with a specific id
  const classId = req.params.cid;
  const userId = req.userId; // Assuming you have access to the user's ID

  const classes = DUMMY_CLASSES.find((c) => {
    return c.id === classId;
  });

  if (!classes) {
    throw new HttpError("Could not find a class for the provided id.", 404);
  }

  // Check if the user is already a member of the class
  if (classes.members.includes(userId)) {
    throw new HttpError("User is already a member of the class.", 400);
  }

  // Add the user to the class's list of members and save the updated class data
  classes.members.push(userId);
  // Save the updated class data to the database here (if using a database)

  res.json({ class: classes });
};
